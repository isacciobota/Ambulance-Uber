...
import { getParamedics } from '../services/loadParamedics';
...
export default function ViewParamedicScreen() {
  ...
  const [Paramedics, setParamedics] = useState(window.paramedics);
  let mounted=true;
  getParamedics().then(d=>{ if(mounted) setParamedics(d)})
  return (
